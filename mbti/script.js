...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function showMatches(mbti) {
        ...
        // 전체 MBTI 궁합 페이지 보기 체크박스 추가
        html += `<li>
                    <input type="checkbox" id="view-all-matches" value="view-all-matches">
                    <label for="view-all-matches"><strong>전체 MBTI 궁합 페이지도 같이 보기</strong></label>
                </li>`;
        ...
        document.getElementById('select-all').addEventListener('change', (event) => {
            ...
            document.getElementById('view-all-matches').checked = isChecked;
        });
    
        button.addEventListener('click', () => {
            const selectedMatches = matches.filter((_, index) => document.getElementById(`match-${index}`).checked);
            const selectedTypes = selectedMatches.map(match => match.type);
            const viewAllMatches = document.getElementById('view-all-matches').checked;
